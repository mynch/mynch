...
method log_data_service {
    my $ls = Mynch::Livestatus->new( config => $self->stash->{config}->{ml} );

    my $since = time() - 172800;    # 2 days of log data
...
method host_downtime_data {
    ...
    my @columns
        = qw{ author comment end_time start_time is_service service_display_name };
...
method host_comments_data {
    ...
    my @columns
        = qw{ author comment entry_time entry_type id is_service service_display_name };
...
method service_comments_data {
    ...
    $query .= sprintf( "Columns: %s\n", join( " ", @columns ) );
    $query .= "Filter: host_name =~ " . $self->stash->{show_host} . "\n";
    $query .= "Filter: service_display_name =~ "
        . $self->stash->{show_service} . "\n";
    $query .= "Filter: entry_type = 1\n";    # user-set comments only
...
method hostgroup_context {
    ...
    if ( scalar @{ $entry->{groups} } > 0 ) {
        foreach my $hostgroup ( @{ $entry->{groups} } ) {
            ...
            my $results_ref = $ls->fetch($query);
            my $hostgroup_context_ref
                = $ls->massage( $results_ref, \@columns );
...
sub dostuff {
    ...
    if ( $host && $service && $comment && $submit ) {
        my $ls = Mynch::Livestatus->new(
            config => $self->stash->{config}->{ml} );
        if ( $submit eq "AddComment" ) {
            $ls->send_commands(
                "ADD_SVC_COMMENT;$host;$service;1;$nick;$comment");
        }
            ...
            my $command
                = "SCHEDULE_SVC_DOWNTIME;$host;$service;$now;$end;1;0;0;$nick;$comment\n";
        ...
        if ( $submit eq "Ack" ) {
            $ls->send_commands(
                "ACKNOWLEDGE_SVC_PROBLEM;$host;$service;1;$notify;1;$nick;Ack."
            );
        }
            ...
            $ls->send_commands(
                "SCHEDULE_FORCED_SVC_CHECK;$host;$service;$now");
        }
    }
        ...
        if ( $submit eq "Ack" ) {
            $ls->send_commands(
                "ACKNOWLEDGE_HOST_PROBLEM;$host;1;$notify;1;$nick;Ack.");
        }
        elsif ( $submit eq "UnAck" ) {
...
sub parse_duration {
    ...
    if ( $s =~ /^(\d{4})-(\d+)-(\d+)(?:T(\d+):(\d+)(?::(\d+))?)?$/ ) {
        ( $year, $mon, $mday, $hour, $min, $sec )
            = ( $1, $2, $3, $4, $5, $6 );
        my $then
            = timelocal( $sec || 0, $min || 0, $hour || 0, $mday, $mon - 1,
            $year );
        $secs = $then - $now;
        $secs = 0 if $secs < 0;
    }
        ...
        do {
            $offset += 86400;
            ( $mday, $mon, $year, $wday )
                = ( localtime( $now + $offset ) )[ 3, 4, 5, 6 ];
            } while ( $wday == 6 || $wday == 0 )
            ;    # TODO: nice-to-have... Use Date::Holidays and country code
